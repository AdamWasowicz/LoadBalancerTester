...
export function setup() {
    const res = http.post(aE.SeedEndpointRoute(aE.ControllersName.WORKER, amountOfSeededItems))
    return;
}
...
export default () => {
    const res = http.get(aE.GetEndpointRoute(aE.ControllersName.WORKER));
    check(res, {
