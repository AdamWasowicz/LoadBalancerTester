...
export function setup() {
    const res = http.post(aE.SeedEndpointRoute(aE.ControllersName.PRODUCT_SOLD, amountOfSeededItems))
    return;
}
...
export default () => {
    const res = http.get(aE.GetEndpointRoute(aE.ControllersName.PRODUCT_SOLD));
    check(res, {
