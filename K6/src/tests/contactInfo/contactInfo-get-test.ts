...
export function setup() {
    const res = http.post(aE.SeedEndpointRoute(aE.ControllersName.CONTACT_INFO, amountOfSeededItems))
    return;
}
...
export default () => {
    const res = http.get(aE.GetEndpointRoute(aE.ControllersName.CONTACT_INFO));
    check(res, {
