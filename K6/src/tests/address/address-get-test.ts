...
export function setup() {
    const res = http.post(aE.SeedEndpointRoute(aE.ControllersName.ADDRESS, amountOfSeededItems))
    return;
}
...
export default () => {
    const res = http.get(aE.GetEndpointRoute(aE.ControllersName.ADDRESS));
    check(res, {
