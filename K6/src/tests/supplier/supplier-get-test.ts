...
export function setup() {
    const res = http.post(aE.SeedEndpointRoute(aE.ControllersName.SUPPLIER, amountOfSeededItems))
    return;
}
...
export default () => {
    const res = http.get(aE.GetEndpointRoute(aE.ControllersName.SUPPLIER));
    check(res, {
